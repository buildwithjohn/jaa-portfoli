...
export default function Hero() {
  return (
    <section id="hero" style={{
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", width:"100%", position:"relative", zIndex:1, display:"grid", gridTemplateColumns:"1fr auto", gap:"5rem", alignItems:"center" }}>
        ...
        <div>
          ...
          {/* Name */}
          <motion.h1 {...fadeUp(0.22)} style={{ fontFamily:"'Syne', sans-serif", fontSize:"clamp(2.6rem,5.5vw,4.8rem)", fontWeight:800, lineHeight:1.06, letterSpacing:"-0.035em", color:"var(--text)", marginBottom:"0.6rem" }}>
            John Ayomide<br />
          ...
          <motion.div {...fadeUp(0.42)} style={{ display:"flex", flexWrap:"wrap", gap:"0.55rem", marginBottom:"1.6rem" }}>
            {[
              ...
              { icon:"🎓", label:"Master's Degree in IT",  color:"rgba(37,99,235,0.08)", border:"rgba(37,99,235,0.2)",  text:"var(--cyan)" },
            ].map((c) => (
              <div key={c.label} style={{ display:"inline-flex", alignItems:"center", gap:"0.45rem", padding:"0.38rem 0.85rem", background:c.color, border:`1px solid ${c.border}`, borderRadius:"8px" }}>
                ...
                <span style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:"0.68rem", color:c.text, letterSpacing:"0.06em", fontWeight:600 }}>{c.label}</span>
              </div>
            ))}
          </motion.div>

          {/* Bio */}
          <motion.p {...fadeUp(0.48)} style={{ fontSize:"1.02rem", color:"var(--text2)", maxWidth:540, lineHeight:1.85, fontWeight:300, marginBottom:"2.2rem" }}>
          ...
          <motion.div {...fadeUp(0.56)} style={{ display:"flex", flexWrap:"wrap", gap:"0.9rem", marginBottom:"2.8rem" }}>
            <motion.a whileHover={{ y:-2, boxShadow:"var(--shadow-cyan)" }} href="#projects"
            ...
            <motion.a whileHover={{ y:-2, borderColor:"var(--cyan)", color:"var(--cyan)" }} href="#contact"
              style={{ padding:"0.85rem 2rem", border:"1.5px solid var(--border)", color:"var(--text2)", fontFamily:"'Syne', sans-serif", fontWeight:600, fontSize:"0.92rem", textDecoration:"none", borderRadius:"10px", background:"var(--surface)", boxShadow:"var(--shadow-sm)", display:"inline-block", transition:"all 0.2s" }}>
              Get in Touch
            </motion.a>
          </motion.div>

          {/* Stats */}
          <motion.div {...fadeUp(0.66)} style={{ display:"flex", flexWrap:"wrap", gap:"2.5rem", paddingTop:"2rem", borderTop:"1.5px solid var(--border)" }}>
            {stats.map((s) => (
              <div key={s.label}>
        ...
        >
          ...
          <div style={{
            ...
            <Image
              ...
              style={{
                objectFit: "cover",
                objectPosition: "center 18%",  /* pull face up into center */
              }}
              priority
            />
          </div>
