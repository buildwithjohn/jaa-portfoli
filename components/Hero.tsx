...
export default function Hero() {
  return (
    <section id="hero" style={{ minHeight:"100vh", display:"flex", alignItems:"center", padding:"120px 5% 80px", position:"relative", overflow:"hidden", background:"var(--bg)" }}>
      ...
      {/* Orbs */}
      <motion.div animate={{ scale:[1,1.1,1], opacity:[1,0.7,1] }} transition={{ duration:8, repeat:Infinity, ease:"easeInOut" }}
        style={{ position:"absolute", top:"-100px", right:"-100px", width:600, height:600, borderRadius:"50%", background:"radial-gradient(circle, rgba(0,200,255,0.1), transparent 70%)", filter:"blur(80px)", pointerEvents:"none" }} />
      <motion.div animate={{ scale:[1,1.12,1], opacity:[1,0.6,1] }} transition={{ duration:10, repeat:Infinity, ease:"easeInOut", delay:2 }}
        style={{ position:"absolute", bottom:0, left:"-100px", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle, rgba(124,111,205,0.08), transparent 70%)", filter:"blur(80px)", pointerEvents:"none" }} />
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", width:"100%", position:"relative", zIndex:1, display:"grid", gridTemplateColumns:"1fr auto", gap:"4rem", alignItems:"center" }}>
        ...
        <div>
          ...
          {/* Name */}
          <motion.h1 {...fadeUp(0.25)} style={{ fontFamily:"'Syne', sans-serif", fontSize:"clamp(2.8rem,6vw,5rem)", fontWeight:800, lineHeight:1.05, letterSpacing:"-0.03em", color:"white", marginBottom:"0.5rem" }}>
            John Ayomide<br/>
          ...
          <motion.p {...fadeUp(0.5)} style={{ fontSize:"1.02rem", color:"var(--text2)", maxWidth:560, lineHeight:1.85, fontWeight:300, marginBottom:"2.5rem" }}>
            I <strong style={{ color:"var(--text)", fontWeight:500 }}>build cloud infrastructure</strong>, design systems that scale, craft visual identities that stand out, and train the next generation of tech professionals — all from <strong style={{ color:"var(--text)", fontWeight:500 }}>Lagos, Nigeria</strong>, for the world.
          </motion.p>

          {/* CTAs */}
          <motion.div {...fadeUp(0.62)} style={{ display:"flex", flexWrap:"wrap", gap:"1rem", marginBottom:"2.5rem" }}>
            <motion.a whileHover={{ y:-2, boxShadow:"0 0 50px rgba(0,200,255,0.5)" }}
              href="#projects"
              style={{ padding:"0.85rem 2rem", background:"var(--cyan)", color:"var(--bg)", fontWeight:600, fontSize:"0.92rem", textDecoration:"none", borderRadius:"8px", boxShadow:"0 0 30px rgba(0,200,255,0.3)", display:"inline-block" }}>
              View My Work →
            </motion.a>
            <motion.a whileHover={{ y:-2, borderColor:"var(--cyan)", color:"var(--cyan)" }}
              href="#contact"
        ...
        {/* RIGHT — Photo */}
        <motion.div initial={{ opacity:0, scale:0.92 }} animate={{ opacity:1, scale:1 }} transition={{ duration:0.9, ease:"easeOut", delay:0.5 }}
          ...
          <motion.div animate={{ rotate:360 }} transition={{ duration:20, repeat:Infinity, ease:"linear" }}
            ...
            <span style={{ position:"absolute", top:-4, left:"50%", transform:"translateX(-50%)", width:8, height:8, borderRadius:"50%", background:"var(--cyan)", boxShadow:"0 0 10px var(--cyan-glow)" }} />
          </motion.div>

          {/* Photo circle */}
          <div style={{ width:310, height:310, borderRadius:"50%", overflow:"hidden", position:"relative", border:"3px solid var(--surface2)", flexShrink:0 }}>
            <Image src="/john.jpg" alt="John Ayomide Akinola" fill style={{ objectFit:"cover", objectPosition:"center top" }} priority />
          </div>

          {/* Floating badges */}
          {[
            { text:"AWS Certified", dot:"var(--green)", top:"10px", right:"-35px", delay:0 },
            { text:"⚡ K8s Expert", dot:"var(--cyan)", bottom:"-10px", right:"-42px", delay:1 },
            { text:"🎨 Photoshop Pro", dot:"#f0b429", bottom:"55px", left:"-45px", delay:0.5 },
          ].map((b,i) => (
            <motion.div key={b.text}
              animate={{ y:[0, i%2===0 ? -8 : 8, 0] }}
              transition={{ duration:4+i, repeat:Infinity, ease:"easeInOut" }}
              style={{ position:"absolute", top:b.top, right:b.right, bottom:b.bottom, left:b.left, padding:"0.45rem 0.8rem", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"8px", fontFamily:"'JetBrains Mono', monospace", fontSize:"0.7rem", color:"var(--text2)", whiteSpace:"nowrap", backdropFilter:"blur(10px)" }}>
