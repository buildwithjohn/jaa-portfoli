      ...
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        ...
        <div className="about-grid">
          ...
          <motion.div initial={{ opacity:0, x:-24 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} transition={{ duration:0.65 }}>
            {[
              <span key="1">I started out studying <strong style={{ color:"var(--text)", fontWeight:500 }}>Chemistry Education</strong> — which gave me something most engineers lack: the ability to break down the complex and make it genuinely understandable.</span>,
            ...
            ].map((p,i)=>(<p key={i} style={{ color:"var(--text2)", fontWeight:300, lineHeight:1.9, fontSize:"0.97rem", marginBottom:"1.2rem" }}>{p}</p>))}
            ...
            <div style={{ position:"relative", paddingLeft:"1.8rem", borderLeft:"1px solid", borderImage:"linear-gradient(to bottom,var(--cyan),transparent) 1" }}>
              {timeline.map((t,i)=>(
                <motion.div key={t.role}
                  ...
                  <span style={{ position:"absolute", left:"-2.15rem", top:"6px", width:12, height:12, borderRadius:"50%", background:"var(--cyan)", boxShadow:"0 0 8px var(--cyan-glow)", display:"inline-block" }}/>
