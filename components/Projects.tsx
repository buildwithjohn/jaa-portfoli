...
const projects = [
  ...
  { num:"03", cat:"Security", title:"PaySecure Bank", desc:"Deliberately vulnerable Nigerian banking application with 20+ documented vulnerabilities, built for ethical hacking assessment and training.", tags:["Ethical Hacking","Pen Testing","OWASP","Training"], link:"https://github.com/Johnprexy" },
...
export default function Projects() {
  return (
    <section id="projects" style={{ padding:"6rem 5%", background:"var(--bg2)" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:"1.4rem" }}>
          {projects.map((p, i) => (
            <motion.div key={p.title}
              initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.5, delay:(i%3)*0.1 }}
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:"0.4rem", marginBottom:"1.2rem" }}>
                {p.tags.map((tag) => (
                  <span key={tag} style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:"0.64rem", color:"var(--cyan)", background:"rgba(0,200,255,0.06)", border:"1px solid var(--border)", borderRadius:"4px", padding:"0.2rem 0.5rem" }}>{tag}</span>
                ))}
              </div>
              <a href={p.link} target={p.link.startsWith("http") ? "_blank" : undefined} rel="noreferrer"
                style={{ display:"inline-flex", alignItems:"center", gap:"0.4rem", fontFamily:"'JetBrains Mono', monospace", fontSize:"0.75rem", color:"var(--text3)", textDecoration:"none", transition:"color 0.2s" }}
